...
- `Атакующий звук / каналы A-B`
- `Голос / Soundcore Motion 300`
- `Лидар`
- `Датчик движения`
...
Нормативный audio-case для этого этапа:

- категория `Аудио` должна различать `attack_audio` и `voice_fx`, а не один плоский `audio` test card;
- `attack_audio` card управляет channel `A` и `B` раздельно, позволяет запускать сценарии и сравнивать локальный черновик с defaults из `Settings`;
- `voice_fx` card проверяет Bluetooth reconnect, playback, microphone path, talkback и effect-preset baseline;
- текущий software skeleton уже материализует saved baseline vs local draft для `attack_audio` и `voice_fx` внутри `Laboratory`; targeted dry-run scenarios можно наращивать поверх этого каркаса без превращения черновика в persisted truth;
- `Сохранить выбор` для audio-профиля никогда не должен молча превращать laboratory tuning в persistent system truth без явного просмотра в `Settings`.
...
- профили дальномеров класса `HC-SR04`, шаговые приводы, эксперименты с пробуждением по движению, прием неизвестных модулей и часть attack-audio / voice-fx профилей остаются локальными для `Laboratory` до явного просмотра и переноса;
