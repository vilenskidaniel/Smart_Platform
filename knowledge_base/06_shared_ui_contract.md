...
- `chat_prompts/foundation_prompt.md` остается prompt-layer companion source для shared wording и guardrails sync.
...
- `degraded`: функция, путь или источник остаются доступными, но работают неполно, с пониженной достоверностью или с потерей части возможностей;
...
- `degraded` не подменяет `fault` и не должен маскировать реальную неисправность;
- `attention` не является автоматическим синонимом `degraded`: это warning/advisory слой, а не всегда отдельная потеря функции;
...
- expected owner, текущая доступность owner-node и route mode не должны пропадать только потому, что поверхность вошла в `blocked` или `degraded` состояние;
- peer-owned поверхность остается видимой и объяснимой, а не маскируется под пустое локальное состояние.
...
## 7. Owner Context And Reports Boundary

Сквозная owner-aware и report-aware семантика должна оставаться одной и той же на всех страницах.

Для owner-context это означает:

- shell cards, module headers, handoff layers и blocked/degraded surfaces должны одинаково показывать, кто владелец и доступен ли он сейчас;
- `local`, `handoff`, `blocked` и related route-modes не должны переопределяться локально на каждой странице как будто это разные сущности;
- peer-missing truth не должна маскироваться под `no data`, если система знает владельца и знает причину недоступности.

### Runtime Identity Separation В Shared UI

Shared shell surfaces обязаны отдельно показывать:

- `viewer`, то есть устройство и браузер, с которого открыта страница сейчас;
- `runtime host`, то есть устройство, где реально работает backend/server;
- `physical nodes`, то есть `Raspberry Pi`, `ESP32` и другие platform nodes из topology;
- `shell surface family`, если он нужен для compatibility wording и routing truth.

Нормативные запреты:

- нельзя показывать `current_shell.node_type` как будто это синоним “текущее устройство пользователя”;
- нельзя выводить имя папки реализации вроде `host_runtime/` как user-facing доказательство того, что пользователь сейчас работает именно с Raspberry Pi;
- нельзя терять различие между `Windows laptop` viewer, bare `Raspberry Pi` display viewer и physical owner node в bar, `Settings`, `Laboratory` и других shared местах.

Нормативный пример:

- если shell family `raspberry_pi` временно поднят на `Windows desktop host`, UI должен одновременно уметь честно показать desktop viewer, desktop host, `Raspberry Pi` physical node и `ESP32` peer node, не подменяя одну сущность другой.

Для границы `Reports` это означает:

- `Gallery > Reports` остается пользовательской историей автономно и системно зафиксированных product-level событий;
- `Laboratory` session notes, console lines, calibration notes, state-table comparisons и другие engineering traces остаются локальным рабочим следом, а не второй формой `Reports`;
- report cards и session-note cards должны оставаться визуально различимыми, даже если используют общие chips, state tokens или timestamp patterns.

## 8. Shared Visual Tokens И Примеры
...
- reusable CSS entry point для этого слоя: `host_runtime/web/static/operator_hud.css`;
- HUD может использовать local radius/panel tokens, но обязан сохранять truthful state и safety visibility.

## 9. Нормативные Примеры Кода И CSS Tokens
