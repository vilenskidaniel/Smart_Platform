(function () {
  ...
  function detectContext(snapshot) {
    ...
    return {
      ...
      peerReachable,
      currentNodeTitle: String(((nodes.current || {}).title) || currentShell.node_type || "Current shell"),
      peerNodeTitle: String(peerNode.title || "Peer board"),
    };
  }
  ...
  function runtimeItem(context) {
    if (context.runtime === "laptop_smoke") {
      return {
        ...
        icon: "desktop",
        label: "Laptop Smoke",
        detail: "Loopback shell detected. This is the local browser-first smoke path for notebook or desktop review. It helps test UI and routing, but it does not replace hardware truth.",
      };
    }
    ...
    if (context.hostType === "esp32") {
      return {
        ...
        icon: "board",
        label: "ESP32 Shell",
        detail: "You are inside the ESP32 shell. Irrigation and local service slices are local here. Turret-owned routes stay owner-aware and should hand off to Raspberry Pi when it is present.",
      };
    }
    ...
    if (context.hostType === "raspberry_pi") {
      return {
        ...
        icon: "board",
        label: "Raspberry Pi",
        detail: "You are inside the Raspberry Pi shell. Turret, display qualification, Gallery, Laboratory, and Settings stay local here. ESP32-owned irrigation routes should hand off or explain the missing owner.",
      };
    }
    ...
    return {
      ...
      icon: "board",
      label: "Browser Shell",
      detail: "The browser shell is active, but the host runtime could not be identified precisely.",
    };
  }
