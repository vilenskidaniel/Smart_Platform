(function () {
  ...
  function entryMetadataPalette(kind) {
    const palettes = {
      desktop: { start: "#294e3b", end: "#427a5d", accent: "#d6f0df", theme: "#2d5d43" },
      phone: { start: "#1f5861", end: "#2f8795", accent: "#d8f5f9", theme: "#2b6d78" },
      tablet: { start: "#5c6928", end: "#8ca54b", accent: "#f1f6ca", theme: "#72863c" },
      display: { start: "#294464", end: "#4675a3", accent: "#dbe9ff", theme: "#375b83" }
    };
    return palettes[normalizeViewerKind(kind)] || palettes.desktop;
  }

  function entryMetadataGlyph(kind) {
    switch (normalizeViewerKind(kind)) {
      case "phone":
        return '<rect x="25" y="14" width="14" height="35" rx="4.8"></rect><path d="M29 20h6"></path><circle cx="32" cy="43" r="1.5" fill="#f7fcf8" stroke="none"></circle>';
      case "tablet":
        return '<rect x="21" y="13" width="22" height="37" rx="4.8"></rect><circle cx="32" cy="44" r="1.4" fill="#f7fcf8" stroke="none"></circle>';
      case "display":
        return '<rect x="16" y="17" width="32" height="21" rx="4"></rect><path d="M26 46h12"></path><path d="M32 38v8"></path>';
      default:
        return '<rect x="16" y="18" width="32" height="20" rx="4"></rect><path d="M27 46h10"></path><path d="M32 38v8"></path>';
    }
  }

  function entryMetadataSvg(kind) {
    const normalized = normalizeViewerKind(kind);
    const palette = entryMetadataPalette(normalized);
    return [
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">',
      '<defs>',
      '<linearGradient id="sp-entry-gradient" x1="8" y1="8" x2="56" y2="56" gradientUnits="userSpaceOnUse">',
      `<stop offset="0%" stop-color="${palette.start}" />`,
      `<stop offset="100%" stop-color="${palette.end}" />`,
      '</linearGradient>',
      '</defs>',
      '<rect x="6" y="6" width="52" height="52" rx="15" fill="url(#sp-entry-gradient)" />',
      '<path d="M16 15c5-4 12-6 20-4 6 1 10 5 14 10-4-1-8 0-11 3-4 4-4 9-3 13-5-2-10-5-14-10-4-5-6-8-6-12Z" fill="#ffffff" opacity="0.12" />',
      `<circle cx="48" cy="16" r="4" fill="${palette.accent}" opacity="0.94" />`,
      '<g fill="none" stroke="#f7fcf8" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round">',
      entryMetadataGlyph(normalized),
      '</g>',
      '</svg>'
    ].join("");
  }

  function entryMetadataHref(kind) {
    return `data:image/svg+xml,${encodeURIComponent(entryMetadataSvg(kind))}`;
  }

  function ensureEntryLink(id, rel) {
  ...
  function ensureEntryMeta(id, name) {
  ...
  function applyEntryMetadata(snapshot) {
    const viewer = detectClient(snapshot);
    const kind = normalizeViewerKind(viewer.kind);
    const title = `${BASE_PAGE_TITLE} / ${viewer.title}`;
    const metadataKey = `${title}|${kind}`;

    if (appliedEntryMetadataKey === metadataKey) {
      return;
    }

    appliedEntryMetadataKey = metadataKey;
    document.title = title;

    const palette = entryMetadataPalette(kind);
    const iconHref = entryMetadataHref(kind);
    const favicon = ensureEntryLink("smart-platform-entry-favicon", "icon");
    const shortcutIcon = ensureEntryLink("smart-platform-entry-shortcut", "shortcut icon");
    const appleTouchIcon = ensureEntryLink("smart-platform-entry-apple-touch", "apple-touch-icon");
    ...
    ensureEntryMeta("smart-platform-entry-theme", "theme-color").content = palette.theme;
    ensureEntryMeta("smart-platform-entry-name", "application-name").content = title;
    ensureEntryMeta("smart-platform-entry-apple-title", "apple-mobile-web-app-title").content = title;
    ensureEntryMeta("smart-platform-entry-capable", "mobile-web-app-capable").content = "yes";
    ensureEntryMeta("smart-platform-entry-apple-capable", "apple-mobile-web-app-capable").content = "yes";
  }
