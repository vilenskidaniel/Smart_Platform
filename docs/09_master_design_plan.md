...
## 1.2. Правило Глубины Документации

На этом этапе важно не спутать две разные задачи:

- сделать документацию удобной для чтения новым чатом;
- случайно выкинуть глубину ради более короткого списка файлов.

Мы выбираем первый путь, но не второй.

Это означает:

- у папки `docs/` должен быть один явный вход и понятный reading order;
- верхние канонические документы должны задавать общую модель продукта и архитектуры;
- глубокие модульные документы должны оставаться, если именно в них живут подробные сценарии, поля, layout, safety-ограничения и инженерные детали;
- stage-docs не удаляются автоматически только потому, что они старые по номеру или написаны как промежуточные заметки;
- любое упрощение структуры допустимо только после явной миграции смысла без потерь.

Практический вывод:

- цель не в минимальном количестве файлов;
- цель в такой структуре, при которой новый чат сначала быстро понимает систему, а потом может безопасно углубиться в нужный слой без поиска скрытой истины по всему репозиторию.

## 1.3. Антираздувание документации

После появления канонического входа `docs/README.md` и deep-docs для ключевых продуктовых разделов базовым действием считаем уже не добавление новых файлов, а уплотнение активного reading path.

Это означает:

- новый документ допустим только если он закрывает уникальный knowledge-gap, который нельзя аккуратно встроить в уже существующий canonical core, deep-doc или supporting map;
- если продуктовая истина уже стабильно живет в canonical core или deep-doc, supporting и stage-docs не должны пересказывать ее своими словами как равноправный источник;
- при конфликте между primary doc и supporting/stage-doc исправляем supporting/stage-doc, а не размножаем параллельные формулировки;
- следующий признак зрелости `docs/` — не рост числа файлов, а сокращение числа мест, где один и тот же смысл приходится перепроверять повторно.
...
Дополнительный вход для всех следующих чатов после этой переделки:

- [README.md](/c:/Users/vilen/OneDrive/Dokumentumok/PlatformIO/Projects/Smart_Platform/docs/README.md)
...
1. возврат к общей координации;
2. real hardware docs;
3. live integration;
4. итоговая уборка проекта и donor-layer cleanup после подтвержденной выборочной миграции.
