...
- отображение `strobe_bench` в общем `Service/Test` контуре.

## Что это означает для Service/Test

`strobe_bench` больше не является просто набором отдельных POST-команд.

Текущая страница уже считается первым углубленным laboratory slice внутри `Service/Test`:
