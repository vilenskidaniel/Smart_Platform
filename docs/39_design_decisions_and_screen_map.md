...
- legacy stage alias `service_test` может сохраняться в compatibility-notes, но user-facing имя страницы фиксируем как `Laboratory`.

Важно дополнительно:

- detailed IA, field-model и session/evidence semantics для `Laboratory`, `Gallery` и `Settings` теперь живут в `docs/50_laboratory_v1_workspace_spec.md`, `docs/51_gallery_v1_content_and_reports_spec.md` и `docs/52_settings_v1_persistent_system_spec.md`;
- этот документ должен держать screen-map decisions и summary-level product agreements, а не заново становиться deep-spec слоем.
...
Подробная workspace-спецификация `Laboratory`, field model, session/evidence layer и save semantics живут в `docs/50_laboratory_v1_workspace_spec.md`.

Здесь фиксируем только screen-map decisions, которые должны быть видны на уровне структуры страницы и рабочего ритма.
...
- console/state-table region как secondary evidence layer;
