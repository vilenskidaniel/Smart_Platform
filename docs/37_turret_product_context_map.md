...
### 4.1 Продуктовый экран турели

Экран:

- `/turret`

Он должен быть ориентированным на оператора и не перегруженным.

Его задача:

- показывать живой FPV-поток;
- показывать текущий режим;
- показывать готовность сенсоров и каналов действия;
- давать быстрое управление в `Manual`;
- давать понятную картину поведения в `Automatic`.

### 4.2 Операторский FPV-слой

В `Manual` и `Automatic` пользователь видит:

- живое видео;
- прицел;
- правдивую рамку зрения и цели; она скрывается, пока слежение зрения или цели симулируется,
  недоступно или неактивно;
- статус readiness важных элементов.

Для `strobe` фиксируем правило доступа:

- доступ продуктового уровня к `strobe` существует из `Manual FPV` как к каналу действия;
...
- два входа обслуживают разные сценарии:
  ...
  - `Laboratory` для проверки, первичного ввода в работу и диагностики.
