# Smart Platform Docs Map

Этот файл является канонической точкой входа в папку `docs/`.

Его цель:

- объяснить, как читать документацию сверху вниз;
- не допустить потери глубины при переписывании структуры;
- сделать папку `docs/` достаточной для нового чата, который должен суметь спроектировать и заново реализовать систему без скрытых обязательных источников.

Важно:

- каноническая документация не означает "короткая документация";
- уменьшение числа файлов само по себе не является целью;
- если в отдельном документе есть уникальная инженерная истина, сценарий, ограничение или полезная детализация, этот документ нельзя выбрасывать, пока смысл не перенесен в новый канонический слой явно и без потерь;
- стартовый reading order нужен для экономии контекстного окна, но не заменяет глубокие модульные и архитектурные документы.
