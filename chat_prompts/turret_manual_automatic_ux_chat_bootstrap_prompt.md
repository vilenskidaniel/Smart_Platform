...
- live video;
- crosshair;
- `vision gate` is a truthful machine-vision overlay, not decoration: hide it
  while vision/target tracking is simulated, offline, or inactive; when active,
  render it as a soft bracket/box without persistent text that blocks the camera
  feed.
...
- Treat that as the shared `operator-hud` pattern for FPV and future Laboratory
  operator panels: reuse the token family instead of copying one-off styles per
  page.
- Current reusable CSS entry point: `raspberry_pi/web/static/operator_hud.css`;
  opt in with `class="operator-hud"` and keep page-specific geometry local.
- верхняя правая строка статусов;
- верхняя левая зона с одним switch `Manual / Automatic`;
- нижняя левая зона с крупным стиком, который визуально отражает keyboard/touch vector;
- клавиатурные направления дают максимальную скорость и складываются в диагонали, touch/pointer внутри стика дает плавную скорость по расстоянию от центра;
- нижняя зона с irrigation valve overlay;
- нижняя правая зона с полукругом action controls под правый палец;
- единая photo/video capture-кнопка: tap = photo, hold = video start, повторное нажатие = video stop;
- video auto-stop берется из `turret_policies.max_recording_seconds` (`30 s` default, `5 s` min, `10 h` max);
- `turret_policies.allow_auto_capture` controls whether the same capture button
  ...
  hidden there.
- capture должен создавать content artifact, появляться в `Gallery > Media` и оставлять запись в `Gallery > Reports`;
- если bar-панель отключает `input helpers / desktop controls`, Manual FPV обязан отключить keyboard, joystick, weapons, valves и capture;
- справа в FPV уместна компактная Betaflight-like telemetry: lidar/range, counters per weapon session, capture counters и input state;
