...
- Speaker/audio token в bar-панели тоже читает и пишет общую persistent truth из `Settings` для `volume / mute / silent mode`, а не invent-ит отдельный local mixer state.
- Быстрый audio panel по клику на speaker должен быть коротким on-demand слоем наподобие desktop speaker sheet и не должен превращать bar в постоянную вторую строку.
